    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.1:
            for elem in sample.keys():
                ...
                flagval = cv2.INTER_NEAREST
                rotated = cv2.warpAffine(tmp, M, (w, h),flags=flagval)
                sample[elem] = tmp
...
class RGBShift(object):
    ...
    def __init__(self, shift_limit=10, img_elem = 'image' ):
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.75:
            ...
            tmp = sample[self.img_elem]

            tmp = np.asarray(tmp, dtype = np.uint8)
            R = tmp[:,:,0] + random.randrange(-1, 1) * self.shift_limit
            G = tmp[:,:,1] + random.randrange(-1, 1) * self.shift_limit
            B = tmp[:,:,2] + random.randrange(-1, 1) * self.shift_limit
            tmp[:, :, 0] = R
            tmp[:, :, 1] = G
            tmp[:, :, 2] = B
...
class IOGPoints(object):
    ...
    """
    def __init__(self, sigma=10, img_elem = 'crop_image' , elem='crop_gt',pad_pixel =10):
        self.sigma = sigma
        self.elem = elem
    ...
    def __call__(self, sample):
        ...
        _img = sample[self.img_elem]

        image = np.asarray(_img, dtype= np.uint8)



        targetshape=_target.shape
        if np.max(_target) == 0:
            sample['IOG_points'] = np.zeros([targetshape[0],targetshape[1],3], dtype=_target.dtype) #  TODO: handle one_mask_per_point case
        else:
            _points = helpers.iog_points(_target, self.pad_pixel)
            sample['IOG_points'] = helpers.make_gt(image, _target, _points, sigma=self.sigma, one_mask_per_point=False)
