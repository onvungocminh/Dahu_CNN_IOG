...
nInputChannels = 6  # Number of input channels (RGB + heatmap of IOG points)
...
# load pretrain_dict
pretrain_dict = torch.load(os.path.join(save_dir, 'models', modelName +  '_epoch-71.pth'))
print("Initializing weights from: {}".format(
    os.path.join(save_dir, 'models', modelName + '_epoch-71.pth')))
...
with torch.no_grad():
    for ii, sample_batched in enumerate(testloader):
        ...
        # y = torch.Tensor.cpu(inputs).detach().numpy()[0, 0, :, :]
        # y1 = torch.Tensor.cpu(inputs).detach().numpy()[0, 4, :, :]
        #
        # y2 = y + y1
        # y3 = y2/np.max(y2) *255
        # cv2.imwrite('/media/minh/MEDIA/Study/deeplearning/interactive_segmentation/iog/Inside-Outside-Guidance/run/Results/'+ str(ii) + '.png', np.asarray(y3,dtype=np.uint8))
        ...
        result = np.array(result * 255, dtype="uint8")  # convert to uint8
        cv2.imwrite(os.path.join(save_dir_res_list[0], metas['image'][0] + '-' + metas['object'][0] + '.png'), result)
